...
import com.wangduoyu.lib.duoyuplayerlib.utils.CommonUtil;
import com.wangduoyu.lib.duoyuplayerlib.utils.FileUtils;
import com.wangduoyu.lib.duoyuplayerlib.utils.StorageUtils;
...
public class DuoYuProxyCacheManager implements ICacheManager, CacheListener {
    ...
    private static DuoYuProxyCacheManager proxyCacheManager;
     ...
     * 单例管理器
     */
    public static synchronized DuoYuProxyCacheManager instance() {
        if (proxyCacheManager == null) {
            proxyCacheManager = new DuoYuProxyCacheManager();
        }
    ...
    public ICacheAvailableListener getCacheAvailableListener() {
        return cacheAvailableListener;
    }
     ...
     */
     static HttpProxyCacheServer getProxy(Context context) {
        HttpProxyCacheServer proxy = DuoYuProxyCacheManager.instance().proxy;
        return proxy == null ? (DuoYuProxyCacheManager.instance().proxy =
                DuoYuProxyCacheManager.instance().newProxy(context)) : proxy;
    }
    ...
    public static HttpProxyCacheServer getProxy(Context context, File file) {
        ...
        if (DuoYuProxyCacheManager.instance().mCacheDir != null
                && !DuoYuProxyCacheManager.instance().mCacheDir.getAbsolutePath().equals(file.getAbsolutePath())) {
            //不一致先关了旧的
            HttpProxyCacheServer proxy = DuoYuProxyCacheManager.instance().proxy;
            ...
            //开启新的
            return (DuoYuProxyCacheManager.instance().proxy =
                    DuoYuProxyCacheManager.instance().newProxy(context, file));
        } else {
            //还没有缓存文件的或者一致的，返回原来
            HttpProxyCacheServer proxy = DuoYuProxyCacheManager.instance().proxy;

            return proxy == null ? (DuoYuProxyCacheManager.instance().proxy =
                    DuoYuProxyCacheManager.instance().newProxy(context, file)) : proxy;
        }
    }
